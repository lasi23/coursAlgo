...
function countSmarties(tableau2D){
    let smartiesParBoite = [];
    for(let i= 0; i < tableau2D.length; i++){
        let som = 0;
        for(let j = 0; j < tableau2D[1].length; j++){
            som += tableau2D[i][j];
        }
        smartiesParBoite.push(som);
    }   
    return smartiesParBoite;
}
let smartiesParBoite = countSmarties(data);

console.log(smartiesParBoite);
...
function numberMiniSmarties(tableau){
    let min = tableau[0];
...
console.log(numberMiniSmarties(smartiesParBoite));




// Et le nombre maximum ?
function numberMax(tableau){
let max = tableau[0];
    for(let i = 1; i < tableau.length; i++){
        if(tableau[i] > max){
            max = tableau[i];
...
console.log(numberMax(smartiesParBoite));

// Combien y a-t-il de Smarties® bleus en tout ?
let ROSE = 0, ROUGE = 1, ORANGE = 2, JAUNE = 3, VERT = 4, BLEU = 5, VIOLET = 6, MARRON = 7;

function numberBlue(tableau){
    let countBlue = 0;
    for(let i = 0; i < tableau.length; i++){
        countBlue += tableau[i][BLEU];
    }
    return countBlue;
}

console.log(numberColorX(data));



function numberColorX(tableau){
    let countColorX = 0;
    for(let j = 0; j < tableau.length; i++){
        countColorX += tableau[i][j];
    }
    return countColorX;
}
...
function numberColor(tableau){
    let tabColorX = [];
    for(let i = 0; i < tableau.length; i++){
        numberColorX();
        tabColorX.push(tableau[i]);
    }
    return tabColorX;
}
let tabColorX = numberColor(data);
console.log(tabColorX);
